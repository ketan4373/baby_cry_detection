...
def main():
    ...
    parser.add_argument('--load_path_data',
                        default='%s/../test' % os.path.dirname(os.path.abspath(__file__)))
    ...
    for file in os.listdir(load_path_data):
        print file
        ####################################################################################################################
        # Set up logging
        ####################################################################################################################
        ...
        logging.info('Reading {0}'.format(file))
        start = timeit.default_timer()

        # Read signal (first 5 sec)
        file_reader = Reader(os.path.join(load_path_data, file))
